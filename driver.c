...
int delete_entry(struct device dev);
int create_entry(struct device dev);
...
void write_sample_data(struct pci_dev *pdev)
{
    ...
    v_buffer = dma_alloc_coherent(&(pdev->dev), 1024 * 1024 * 4, &p_buffer, GFP_KERNEL);
    if(v_buffer == NULL)
    {
	    printk(KERN_INFO "Error in dma allocation\n");
    }

    /* Write 32-bit data to the device memory */
    iowrite32(0x300, drv_priv->hwmem);
    iowrite32((unsigned int)p_buffer, &(((unsigned char*)drv_priv->hwmem)[100]));
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("credits to someone");
